"""extract text from the 11 Career Center PDF guides and load them as chunks"""
...
from config import DB_PATH, CAREER_GUIDES_DIR

# 4000 chars is enough to fit one full guide page in most cases without
# bumping into the model's context budget for retrieved chunks
...
def chunk_text(text: str, page_num: int) -> list[tuple[int, int, str]]:
    ...
    while start < len(text):
        ...
        # try to break on a paragraph boundary so the chunk reads cleanly
        if end < len(text):
            for sep in ["\n\n", "\n", ". "]:
